...
javac Validate.java || goto CompValError
...
rem call jar cf Check.jar *.class
...
for %%i in (tests\??) do (
    ...
    if EXIST %problem%_%author%.exe (call %problem%_%author%.exe < %problem%.in > %problem%.out || goto SourceRunningError)
    if EXIST %problem%_%author%.class (call java -Xmx256M -Xss64M -ea %problem%_%author% < %problem%.in > %problem%.out || goto SourceRunningError)
    if EXIST %problem%_%author%.py (call python %problem%_%author%.py < %problem%.in > %problem%.out || goto SourceRunningError)
