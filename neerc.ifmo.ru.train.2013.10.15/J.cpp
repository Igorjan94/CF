...
double integrate(double l, double u)
{
    return (-exp(-u*u/2/si/si)+exp(-l*l/2/si/si));
}

void run()
{
    double rbi, rbo, rti, rto, rdi, rdo;
    cin >> rbi >> rbo >> rti >> rto >> rdi >> rdo;
    cin >> si;

    double bp = integrate(0,rbi);
    double mbp = integrate(rbi,rbo);
    double x1p = integrate(rbo,rti)+integrate(rto,rdi);
    double x2p = integrate(rdi,rdo);
    double x3p = integrate(rti,rto);

    double ov = (bp*50+mbp*25+x1p*10.5+x2p*21+x3p*31.5);//(bp+mbp+x1p+x2p+x3p);
    printf("%.6f\n", ov);
}
