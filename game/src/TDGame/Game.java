...
public class Game extends Canvas implements Runnable {
	...
	public void render() {
		...
		if(fullScreen) {
			AffineTransform mt = AffineTransform.getScaleInstance((float) (scrWidth - 25) / WIDTH, (float) (scrHeight - 100) / HEIGHT);
			g.setTransform(mt);
		}
		//рисовач
		...
		if(showInfo == true) {
			...
			if(wawes.getMode()) {
				g.drawString("Status: " + (wawes.getWaweNumber() + 1) + "/" + (char)8734, 915, 165);
			} else {
				g.drawString("Status: " + (wawes.getWaweNumber() + 1) + "/" + wawes.getWaveCount(), 915, 165);
			}
