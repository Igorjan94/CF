...
void run()
{
    ...
    vector<point<ll>> a(n);
    readln(a);
    map<pair<ll, ll>, map<pair<ll, ll>, map<ll, int>>> m;
    auto norm = [](ll& q, ll& w) {
    ...
    fori(n)
        FOR(j, i + 1, n)
        {
            ...
            ll x = qq.x, y = qq.y;
            norm(x, y);
            ll q = x * a[i].y - y * a[i].x;
            ll w = x;
            if (w)
                norm(q, w);
            else
                q = a[i].x;
            //writeln(a[i], a[j], x, y, q, w, !qq);
            m[{x, y}][{q, w}][!qq]++;
        }
    ll ans = 0;
    ll extra = 0;
    for (const auto& [k, v]: m)
    {
        ...
        for (const auto& [t, dists]: v)
        {
            ll temp = 0;
            for (const auto& [dist, c]: dists)
                extraSum += c * 1ll * (cur[dist] - c),
                temp += c;
            ans += temp * 1ll * (sum - temp);
        }
        //writeln(k, sum, extraSum);
        extra += extraSum / 2;
    }
    writeln(ans / 2 - extra / 2);
}
