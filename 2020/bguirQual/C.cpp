...
static const unsigned long long MOD = 4294967296;
typedef modular<unsigned long long, MOD> mod;
...
void run()
{
    ...
    uint N = 1103515245;
    uint M = 12345;
    uint inverse = binpow(N, MOD - 2);

    auto rng = [&]() {
        return seed = (seed * N + M);
    };
    auto invert = [&](mod s) {
        s -= M;
        s *= inverse;
