...
{
    ints(n);
    vector<pair<string, string>> commands(n);
    map<string, vector<int>> firsts, seconds;
    vector<string> ans(n, "X");

    readln(commands);
    fori(n)
        commands[i].first = commands[i].first.substr(0, 3),
        commands[i].second = commands[i].first.substr(0, 2) + commands[i].second.substr(0, 1),
        firsts[commands[i].first].pb(i);
    
    set<string> blocked;
    bool ok = true;
    auto isBlocked = [&](string s) {
        return blocked.find(s) != blocked.end();
    };

    function<void(string)> blockName = [&](string s) {
        blocked.insert(s);
        for (auto& y : firsts[s])
        {
            if (!isBlocked(commands[y].second))
                blockName(commands[y].second);
            ans[y] = commands[y].second;
        }
    };
    ...
    set<string> check;
    fori(n)
        ans[i] = (ans[i] == "X" ? commands[i].first : ans[i]),
        check.insert(ans[i]);
