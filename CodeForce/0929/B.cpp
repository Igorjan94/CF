...
void run()
{
    ...
    vector<string> s(n);
    readln(s);
    vector<tuple<int, int, int>> places;
    int R = 3 + 4 + 3 + 2;
    fori(n)
        forj(R)
            if (s[i][j] == '.')
                places.emplace_back((j > 0) * (s[i][j - 1] == 'S') + (j < R - 1) * (s[i][j + 1] == 'S'), i, j);
    sort(whole(places));
    fori(k)
    {
        auto& [ignore, x, y] = places[i];
        s[x][y] = 'x';
    }
    int count = 0;
    fori(n)
        forj(R)
            if (s[i][j] == 'S')
                count += (j > 0) * (s[i][j - 1] != '-' && s[i][j - 1] != '.') + (j < R - 1) * (s[i][j + 1] != '-' && s[i][j + 1] != '.');
    writeln(count);
    writeln(s);
}
