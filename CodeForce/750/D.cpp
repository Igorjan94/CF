...
void run()
{
    ...
    fori(n)
    {
        ...
        for (auto z : q)
            forj1(a[i] + 1)
            {
                int tx = z.first.first + dirs[z.second].first * j;
                int ty = z.first.second + dirs[z.second].second * j;
                int c = z.second;
                dp[tx][ty] = true;
                if (j == a[i])
                    t.insert({{tx, ty}, nextDirs[c][0]}),
                    t.insert({{tx, ty}, nextDirs[c][1]});
            }
        q = t;
    }
    int ans = 0;
