...
void run()
{
    ...
    vector<string> ss(n);
    readln(ss);
    map<pair<int, char>, vector<string>> strings;
    map<int, vector<string>> orphans;
    for (string& s: ss)
    {
        int cnt = 0;
        char last;
        for (char& c: s)
            if (c == 'a' || c == 'e' || c == 'o' || c == 'u' || c == 'i')
                cnt++,
                last = c;
        strings[{cnt, last}].pb(s);
    }
    for (auto& [p, same]: strings)
        if (SZ(same) & 1)
            orphans[p.first].pb(same.back()),
            same.pop_back();
    vector<pair<string, string>> ans, pairs, odd;
    for (auto& [_, same]: strings)
        for (int i = 0; i + 1 < SZ(same); i += 2)
            pairs.pb({same[i], same[i + 1]});
    for (auto& [_, same]: orphans)
        for (int i = 0; i + 1 < SZ(same); i += 2)
            odd.pb({same[i], same[i + 1]});
    while (pairs.size() > odd.size())
        odd.push_back(pairs.back()),
        pairs.pop_back();
    fori(SZ(pairs))
        ans.pb({odd[i].first, pairs[i].first}),
        ans.pb({odd[i].second, pairs[i].second});
