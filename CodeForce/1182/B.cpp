    ...
    fori(n)
        forj(m)
            if (s[i][j] == '*')
                r[i]++,
                c[j]++;
    int I = max_element(whole(r)) - r.begin();
    int J = max_element(whole(c)) - c.begin();
    auto check = [&]() {
        bool c1 = false, c2 = false, r1 = false, r2 = false;
        FOR(i, I + 1, n) if (s[i][J] == '*') s[i][J] = '.', r1 = true; else break;
        ROF(i, I - 1, 0) if (s[i][J] == '*') s[i][J] = '.', r2 = true; else break;
        FOR(j, J + 1, m) if (s[I][j] == '*') s[I][j] = '.', c1 = true; else break;
        ROF(j, J - 1, 0) if (s[I][j] == '*') s[I][j] = '.', c2 = true; else break;
        bool ok = s[I][J] == '*' && c1 && c2 && r1 && r2;
        s[I][J] = '.';
        fori(n) forj(m) ok &= s[i][j] == '.';
        return ok;
    };
