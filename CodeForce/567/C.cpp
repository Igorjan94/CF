...
{
    ints(n, k);
    vi a(n), countPrev(n), countNext(n);
    ...
    {
        int x = a[i] / k;
        countPrev[i] = a[i] % k == 0 ? prev.find(x) == prev.end() ? 0 : prev[x] : 0;
        prev[a[i]] = prev.find(a[i]) == prev.end() ? 1 : prev[a[i]] + 1;
    }
    ROF(i, n - 1, 0)
    {
        ll x = 1ll * a[i] * k;
        countNext[i] = next.find(x) == next.end() ? 0 : next[x];
        next[a[i]] = next.find(a[i]) == next.end() ? 1 : next[a[i]] + 1;
    }
    fori(n)
        ans += countNext[i] * 1ll * countPrev[i];
    writeln(ans);
}
