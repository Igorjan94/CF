...
void run()
{
    ...
    {
        ints(x, y);
        actions.push_back({x, -1, y, i + 1});
        actions.push_back({y, +1, x, i + 1});
    }
    sort(whole(actions));
    set<pii> current;
    vector<int> ans;
    for (auto& [x, type, y, index]: actions)
        if (type > 0) // end
            current.erase({-x, index});
        else
        {
            current.insert({-y, index});
            if (SZ(current) > k)
            {
                auto [_, i] = *current.begin();
