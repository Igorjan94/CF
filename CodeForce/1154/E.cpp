...
struct dsu
{
    ...
    int getNext(int i)
    {
        return i == SZ(a) || i == a[i] ? i : a[i] = getNext(a[i]);
    }

    int getPrev(int i)
    {
        return i == -1 || i == b[i] ? i : b[i] = getPrev(b[i]);
    }

    void uniNext(int i, int j)
    {
        if (j == SZ(a))
        {
            a[i] = SZ(a);
...
void run()
{
	...
	dsu d(n);
	set<pii> s;
	fori(n) s.insert({-a[i], i});
    char c = '1';
	string ans(n, '0');
	while (!s.empty())
	{
		auto [mx, ind] = *s.begin();
		for (int i = ind, j = 0; j <= k && i < n; ++j, i = d.getNext(i))
            d.uniNext(i, i + 1),
            d.uniPrev(i, i - 1),
            ans[i] = c,
			s.erase({-a[i], i});
        //c++; writeln(ans, s.size()); cout.flush();
        for (int i = ind, j = 0; j <= k && i >= 0; ++j, i = d.getPrev(i))
            d.uniNext(i, i + 1),
            d.uniPrev(i, i - 1),
            ans[i] = c,
            s.erase({-a[i], i});
