...
void run()
{
    ...
    fori(n)
    {
        map<state, mod> states = {{{w[i], add, sub}, mod(1, 1)}};
        forj(m)
        {
            map<state, mod> next;
            for (auto& [s, p]: states)
            {
                auto& [weight, toAdd, toSub] = s;
                int sum = toAdd + toSub;
                if (likes[i])
                {
                    next[{weight + 1, toAdd + 1, toSub}] += p * mod(weight, sum);
                    if (toAdd - weight)
                        next[{weight, toAdd + 1, toSub}] += p * mod(toAdd - weight, sum);
                    if (toSub)
                        next[{weight, toAdd, toSub - 1}] += p * mod(toSub, sum);
                }
                else
                {
                    if (weight)
                        next[{weight - 1, toAdd, toSub - 1}] += p * mod(weight, sum);
                    next[{weight, toAdd + 1, toSub}] += p * mod(toAdd, sum);
                    if (toSub - weight)
                        next[{weight, toAdd, toSub - 1}] += p * mod(toSub - weight, sum);
                }
            }
            states = next;
        }
