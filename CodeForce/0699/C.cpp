...
void run()
{
    ...
    {
        if (i > 2)
            ans = max(dp[1][i - 2], dp[2][i - 2]);
        switch (a[i - 1]) 
        {
            case 0:
                dp[1][i] = dp[2][i] = max(dp[1][i - 1], dp[2][i - 1]);
                if (i > 2)
                    dp[1][i] = dp[2][i] = max(ans, dp[1][i]);
                break;
            case 1:
                dp[1][i] = dp[2][i - 1] + 1;
                if (i > 2)
                    dp[1][i] = max(dp[1][i], ans + 1);
                break;
            case 2:
                dp[2][i] = dp[1][i - 1] + 1;
                if (i > 2)
                    dp[2][i] = max(dp[2][i], ans + 1);
                break;
            default:
                dp[1][i] = dp[2][i - 1] + 1;
                dp[2][i] = dp[1][i - 1] + 1;
