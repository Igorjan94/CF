...
void run()
{
    ...
    vector<int> a(n);
    readln(a);
    vector<int> countOfOnes(m, 0);
    vector<int> curr(m, 0);
    int twos = 0;
    fori(n)
        countOfOnes[i + 1] = countOfOnes[i] + (a[i] == 1),
        twos += a[i] == 2;

    auto getTwos = [&](int i) {
        int left = i - countOfOnes[i];
        int right = twos - left;
        return right;
    };
    int ans = 0;
    ROF(i, n, 0)
    {
        ROF(j, i, 0)
            ...
            {
                dp1[i][j] = dp1[i][j + 1];
                dp2[i][j] = max(dp1[i][j + 1], dp2[i][j + 1]) + 1;
            }
    }
    //fori(m)
        //forj(m)
    ...
    fori(m)
        FOR(j, i, m)
            ans = max(ans, countOfOnes[i] + getTwos(j) + (j == 0 ? 0 : max(dp1[j - 1][i], dp2[j - 1][i])));
