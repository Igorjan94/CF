...
private fun run() {
    ...
    for (i in 0..n - 1)
        a.add(readLine()!!.toLong(2))
    val M = 1L shl m
    val l = Math.max(0, M / 2 - n - 1)
    val r = Math.min(M - 1, M / 2 + n + 1)
    for (i in l..r) {
        var lower = i + 1
        var greater = M - i - 1
        var fail = false
        for (x in a)
            if (x == i)
                ...
                greater--
        if (!fail)
            if (lower == greater || lower == greater + 1) {
                writeln(java.lang.Long.toBinaryString(i).padStart(m, '0'))
                return
            }
    }
    writeln("FAIL")
