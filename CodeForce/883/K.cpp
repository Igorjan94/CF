...
void run()
{
    ...
    vector<pii> a(n);
    readln(a);
    vi x(n, 0), y(n, 0), z(n, 0);
    x[0] = a[0].first + a[0].second;
    y[n - 1] = a[n - 1].first + a[n - 1].second;
    fori1(n)
        x[i] = min(x[i - 1] + 1, a[i].first + a[i].second);
    ROF(i, n - 2, 0)
        y[i] = min(y[i + 1] + 1, a[i].first + a[i].second);
    fori(n)
        z[i] = min(x[i], y[i]);
    bool ok = true;
    ll ans = 0;
    fori(n)
        ok &= a[i].first <= z[i] && z[i] <= a[i].first + a[i].second,
        ans += z[i] - a[i].first;
    if (ok)
        writeln(ans),
        writeln(z);
