...
void run()
{
    ...
    readln(v);
    int count = 0;
    vector<int> cc(26, 0), revealed(26, 0);
    fori(n) if (mask[i] != '*') revealed[mask[i] - 'a'] = 1;

    fori(m)
    {
        bool ok = true;
        forj(n) ok &= (mask[j] == '*' && !revealed[v[i][j] - 'a']) || mask[j] == v[i][j];
        if (ok)
        {
            vector<int> temp(26, 0);
            count++;
            forj(n)
                if (mask[j] == '*') temp[v[i][j] - 'a']++;
            forj(26)
                cc[j] += temp[j] > 0;
        }
    }
    int ans = 0;
    fori(26) ans += cc[i] == count;
    writeln(ans);
}
