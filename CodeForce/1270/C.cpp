...
void run()
{
    ...
    vi a(n);
    readln(a);
    auto getBit = [&](ll x, int i) -> bool {
        return !!(x & (1ll << i));
    };
    auto setBit = [&](ll& x, int i) {
        x |= (1ll << i);
    };
    ...
    ll ans = 0;
    bool lastBit = getBit(sum, 0);
    if (lastBit) setBit(ans, 0);
    bool carry = lastBit;
    fori1(63)
    {
        bool cur = carry ^ getBit(sum, i);
        bool x_bit = getBit(x, i - 1) ^ lastBit;
        lastBit = cur ^ x_bit;
        if (lastBit) setBit(ans, i);
        carry = (carry + lastBit + getBit(sum, i)) >= 2;
    }
    writeln(1);
    writeln(ans);
}
