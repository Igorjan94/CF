...
void printAnswerAndExit(string s)
{
    writeln(s);
    exit(0);
}
...
void run()
{
    ...
    fori(s.size())
        if (s[i] == 'B')
            ...
            g++;
    if (r && g && b)
        printAnswerAndExit("BGR");

    if (r && !g && !b)
        printAnswerAndExit("R");
    if (!r && g && !b)
        printAnswerAndExit("G");
    if (!r && !g && b)
        printAnswerAndExit("B");
    ...
    if (r > 1 && g == 1 && b == 0)
        printAnswerAndExit("BG");
    if (r == 1 && g > 1 && b == 0)
        printAnswerAndExit("BR");

    if (r > 1 && g == 0 && b == 1)
        printAnswerAndExit("BG");
    if (r == 1 && g == 0 && b > 1)
        printAnswerAndExit("GR");
