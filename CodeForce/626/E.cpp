...
double get(vector<int>& a, vector<ll>& pref, int i, int j)
{
    //writeln("--------------");
    //writeln(i, j);
    int n = a.size();
    ...
    if (sz == 0)
        return 0;
    double mediana, medium;
    auto get = [&](int index) {
        return i > index ? a[index] : a[j + index - i];
    };
    //writeln("getter");
    //forn(k, sz)
        //print(get(k));
    ...
    if (sz % 2)
        ...
        mediana = (get(sz / 2) + get(sz / 2 - 1) + 0.0) / 2;
    medium = (pref.back() + pref[i] - pref[j] + 0.0) / sz;
            //FOR(q, 0, i)
                //print(a[q]);
            //FOR(q, j, n)
                //print(a[q]);
            //writeln();
            //wr(sz, mediana, medium);
    return medium - mediana;
}
...
void run()
{
    ...
    int j = -1;
    fori(n - 2)
        if ((temp = a[i] - 2 * a[i + 1] + a.back()) > mx)
            mx = temp,
            j = i;
    writeln(3);
    writeln(a[j], a[j + 1], a.back());
}
