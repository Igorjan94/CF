...
struct fly {
    int d, f, t, c;
    fly(){}
    bool operator<(fly const& b) {
        return d == b.d ? f == b.f ? t == b.t ? c < b.c : t < b.t : f < b.f : d < b.d;
    };
};
...
void run()
{
    ...
    sort(whole(flies));
    map<int, ll> prices;
    set<pair<int, ll>> depatures, leaves;
    ll price = 0;
    ...
    for (fly& f : flies)
        if (f.t == 0)
        {
            ...
            if (!cur || cur > f.c)
                price -= cur - f.c,
                prices[f.f] = f.c;
            if (int(prices.size()) == n)
                depatures.insert({f.d, price});
        }
    price = 0;
    prices.clear();
    reverse(whole(flies));
    ...
    for (fly& f : flies)
        if (f.f == 0)
        {
            ...
            if (!cur || cur > f.c)
                ...
                prices[f.t] = f.c;
            if (int(prices.size()) == n)
                leaves.insert({f.d, price});
        }

    ll III = INF * 1ll * INF;
    ll ans = III;
    for (auto& p : depatures)
    {
        auto temp = leaves.upper_bound({p.first + k, 0});
        if (temp != leaves.end())
            ans = min(ans, p.second + temp->second);
    };
    writeln(ans == III ? -1 : ans);
    //writeln_range(whole(depatures));
    //writeln();
    //writeln_range(whole(leaves));
    //writeln(flies);
}
...
istream&operator>>(istream&is,fly&a){return is>>a.d>>a.f>>a.t>>a.c;}
ostream&operator<<(ostream&os,fly&a){return os<<a.d<<" "<<a.f<<" "<<a.t<<" "<<a.c;}
