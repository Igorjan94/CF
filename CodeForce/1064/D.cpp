...
void run()
{
    ...
    while (q.size())
    {
        auto [i, j, xx, yy] = q.front();
        q.pop_front();
        for (int dx = -1; dx <= 1; ++dx)
            for (int dy = -1; dy <= 1; ++dy)
                if (int u = i + dx, v = j + dy, d2x = dx * dx, d2y = dy * dy, isR = dy == 1, isL = dy == -1, uses = isL * isL + isR * isR == 0; 
                        d2x + d2y == 1 
                            && ok(u, v)
                            && s[u][v] == '.'
                            && xx - isL >= 0 && yy - isR >= 0)
                {
                    s[u][v] = '#';
                    if (uses)
                        q.push_front({u, v, xx - isL, yy - isR});
                    else
                        q.push_back({u, v, xx - isL, yy - isR});
                }
    }
