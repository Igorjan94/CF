...
void run()
{
    ...
    auto getBit = [&](int x, int bit) {
        return !!(x & (1 << bit));
    };

    function<int(vector<int>, int)> get = [&](vector<int> all, int bit) {
        if (bit == -1) return 0;
        vector<int> zeros;
        vector<int> ones;
        for (int x: all)
            (getBit(x, bit) ? ones : zeros).pb(x);

        if (SZ(zeros) == 0)
            return get(ones, bit - 1);
        if (SZ(ones) == 0)
            return get(zeros, bit - 1);
        return (1 << bit) | min(get(zeros, bit - 1), get(ones, bit - 1));
    };
