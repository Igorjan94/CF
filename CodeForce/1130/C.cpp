...
#define y1 sefgsjkdfhgsjkdfgh
int n, x1, y1, x2, y2;
vector<string> s;
vector<vector<bool>> canS;
vector<vector<bool>> canT;

void dfs(vector<vector<bool>>& can, int i, int j)
{
    if (i < 0 || j < 0 || i == n || j == n || s[i][j] == '1' || can[i][j]) return;
    can[i][j] = true;
    dfs(can, i + 1, j);
    dfs(can, i, j + 1);
    dfs(can, i - 1, j);
...
void run()
{
    readln(n, x1, y1, x2, y2); --x1; --y1; --x2; --y2;
    s.resize(n);
    canS.resize(n, vector<bool>(n, false));
    canT.resize(n, vector<bool>(n, false));
    readln(s);
    dfs(canS, x1, y1);
    dfs(canT, x2, y2);
