...
void run()
{
    ...
    auto getDist = [&n](int from, int to) {
        return (to - from + n) % n;
    };
    for (auto& p: where)
    {
        int station = p.first;
        vector<int>& candies = p.second;
        sort(whole(candies), [&](int i, int j) {
            return getDist(station, i) < getDist(station, j);
        });
        if (!candies.empty())
            time[station] = (SZ(candies) - 1) * 1ll * n + getDist(station, candies[0]);
    }
    vector<ll> ans(n, 0ll);
    fori1(n + 1)
        forj1(n + 1)
            if (time[j])
                ans[i - 1] = max(ans[i - 1], time[j] + getDist(i, j));
    writeln(ans); //
}
