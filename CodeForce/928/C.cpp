...
struct package
{
    ...
    package() {}
    package(string name, int version) : name(name), version(version) {}
    operator pair<string, int>() { return {name, version}; }
};
...
istream& operator>>(istream& is, package& p)
{
    ...
    fori(n)
        is >> name >> version,
        p.dependencies.pb({name, version});
    return is;
};
...
void run()
{
    ...
    packages.resize(n);
    readln(packages);
    fori(n) m[packages[i]] = i;
    
    queue<package> q;
    q.push(packages[0]);
    deps[packages[0].name] = packages[0].version;
    while (!q.empty())
    {
        map<string, int> curr;
        while (!q.empty())
        {
            auto dependencies = packages[m[q.front()]].dependencies;
            q.pop();

            for (auto& [name, version, ignore] : dependencies)
                curr[name] = max(curr[name], version);
        }

        for (auto& [name, version] : curr)
            if (deps.find(name) == deps.end())
                deps[name] = version,
                q.push({name, version});
        
    }
    deps.erase(packages[0].name);

    writeln(deps.size());
    writeln_range(whole(deps));
}
