...
struct Node 
{
    ...
    Node operator+(const Node& b) const {
        if (v < b.v)
            return Node(v, pos, l + b.l);
        else
            return Node(b.v, b.pos, l + b.l);
    }
...
void run()
{
    ...
    int s = 0;
    int ans = 0;
    set<pair<int, int>> baught;
    vector mn(n, MOD);
    segmentTree<int, Node, Update> tree(mn);
    fori(n)
    {
        int y = a[i];
        while (baught.size())
        {
            auto [big, i] = *baught.rbegin();
            auto [small, pos, _] = tree.get(i + 1, n - 1);
            if (small < big) {
                baught.erase({big, i});
                s += big;
                s -= small;
                baught.insert({small, pos});
                tree.update(pos, pos, MOD);
            }
            else
                break;
        }
