...
void run()
{
    ...
    vector<int> b(n);
    iota(all(b), 0);
    sparseTable<int> mxa(b, [&](const int& x, const int& y) {
        return a[x] < a[y] ? y : x;
    });
    sparseTable<ll> prevMn(p, [&](const ll& x, const ll& y) { return min(x, y);});
    sparseTable<ll> prevMx(p, [&](const ll& x, const ll& y) { return max(x, y);});
    bool ok = true;
    auto dfs = [&](auto dfs, int l, int r) {
        if (l > r) return;
        int m = mxa.get(l, r);
        ll mn = prevMn.get(l, m);
        ll mx = prevMx.get(m + 1, r + 1);
        if (a[m] < mx - mn)
            ok = false;
        dfs(dfs, l, m - 1);
        dfs(dfs, m + 1, r);
