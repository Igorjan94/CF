...
void run()
{
    ints(n, m, k);
    vector<string> s(n);
    vector<vector<int>> dp(n, vi(k + 1, INTMAX));
    readln(s);
    fori(n)
    {
        int last = -1;
        ...
        vector<int> ans(k + 1, INTMAX);
        forj(m)
            if (s[i][j] == '1')
            {
                count++;
                if (last != -1)
                    prefix.pb(prefix.back() + j - last),
                    curr += j - last;
                else
                    curr++;
                last = j;
        ...
        forj(k + 1)
            for (int q = 0; q <= j; ++q)
                ans[j] = clamp(curr - suffix[q] - prefix[j - q], 0, ans[j]);
        if (i)
            forj(k + 1)
                for (int q = 0; q + j <= k; ++q)
                    dp[i][j + q] = min(dp[i][j + q], dp[i - 1][j] + ans[q]);
        else
            forj(k + 1)
                dp[i][j] = ans[j];
    }
