...
{
    int i, r, c;
    Move(int i, int r, int c) : i(i), r(r), c(c) {}
    Move(){}
};

ostream& operator<<(ostream& os, Move m)
{
    return os << m.i << " " << m.r << " " << m.c;
}
...
void run()
{
    ...
    else
    {
        ...
        auto addMove = [&](int fr, int fc, int tr, int tc, bool add)
        {
            ans.pb(Move(a[fr][fc], tr + 1, tc + 1));
            a[tr][tc] = a[fr][fc];
            a[fr][fc] = 0;
            if (add)
                count++;
        ...
        do
        {
            ...
            fori(n)
            {
                if (a[1][i] && a[1][i] == a[0][i])
                    addMove(1, i, 0, i, false);
                if (a[2][i] && a[2][i] == a[3][i])
                    addMove(2, i, 3, i, false);
            }
            if (n > 1)
            {
                bool firstMoved = false;
                if (a[1][0] != 0 && a[2][0] == 0)
                    addMove(1, 0, 2, 0, true),
                    firstMoved = true;
                fori1(n)
                    if (a[1][i - 1] == 0 && a[1][i] != 0)
                        addMove(1, i, 1, i - 1, true);
                if (a[1][n - 1] == 0 && a[2][n - 1] != 0)
                    addMove(2, n - 1, 1, n - 1, true);
                ROF(i, n - 2, firstMoved)
                    if (a[2][i + 1] == 0 && a[2][i] != 0)
                        addMove(2, i, 2, i + 1, true);
            }
            else
            {
                bool firstMoved = false;
                if (a[1][0] != 0 && a[2][0] == 0)
                    addMove(1, 0, 2, 0, true),
                    firstMoved = true;
                if (!firstMoved && a[1][n - 1] == 0 && a[2][n - 1] != 0)
                    addMove(2, n - 1, 1, n - 1, true);
            }
        }
        while (count);
