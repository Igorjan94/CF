...
void run()
{
    ...
    {
        int temp = cnt[a[i]]++;
        if (temp && --sizes[temp] == 0)
            sizes.erase(temp);
        sizes[temp + 1]++;
        auto [first, value] = *sizes.begin();
        if (SZ(sizes) == 1 && (first == 1 || value == 1))
            ans = i;
        if (SZ(sizes) == 2)
            if (int second = (*sizes.rbegin()).first; (first + 1 == second && sizes[second] == 1) || (first == 1 && sizes[first] == 1))
                ans = i;
    }
    writeln(ans + 1);
}
