...
void run()
{
    ...
    auto upd = [&](int i, int j, int k, int value) {
    ...
    fori(q)
    {
        ...
        if (c == '+')
        {
            ...
            t[x] += d;
            cur[x]++;
            FOR(i, x == 0 ? cur[0] : 0, cur[0] + 1)
                FOR(j, x == 1 ? cur[1] : 0, cur[1] + 1)
                    FOR(k, x == 2 ? cur[2] : 0, cur[2] + 1)
                    {
                        dp[i][j][k] = n + 1;
                        if (i)
                            upd(i, j, k, nxt[ dp[i - 1][j][k] ][ t[0][i - 1] - 'a' ] + 1);
                        if (j)
                            upd(i, j, k, nxt[ dp[i][j - 1][k] ][ t[1][j - 1] - 'a' ] + 1);
                        if (k)
                            upd(i, j, k, nxt[ dp[i][j][k - 1] ][ t[2][k - 1] - 'a' ] + 1);
                    }
        }
        else
        {
        ...
        writeln(dp[cur[0]][cur[1]][cur[2]] <= n ? "YES" : "NO");
    }
}
