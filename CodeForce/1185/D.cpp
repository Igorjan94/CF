...
void run()
{
    ...
    if (n == 2)
        return writeln(1);
    if (n == 3)
        return writeln(a[1].second + 1);
    map<int, int> diffs;
    auto getDiff = [&](int index, int d = 1) {
        return a[index].first - a[index - d].first;
    };
    ...
    {
        int first = getDiff(i);
        int second = getDiff(i + 1);
        int third = getDiff(i + 1, 2);
        dec(first);
        dec(second);
        inc(third);
        if (diffs.size() == 1)
            return writeln(a[i].second + 1);
        inc(first);
        inc(second);
        dec(third);
    }
    {
        ...
        dec(first);
        if (diffs.size() == 1)
            return writeln(a[0].second + 1);
        inc(first);
    }
    {
        int last = getDiff(n - 1);
        dec(last);
        if (diffs.size() == 1)
            return writeln(a.back().second + 1);
    }
    writeln(-1);
}
