...
void run()
{
    ...
    fori(m)
    {
        ...
        sort(whole(pizzas[mask]));
        if (pizzas[mask].size() == 3)
            pizzas[mask].pop_back();
    }
    int price = INTMAX;
    int count = 0;
    pii ans;
    auto updateAns = [&](const pii& a, const pii& b, int mask) {
        int cur = a.first + b.first;
        int cc = 0;
        fori(M)
            if ((mask & i) == i)
                cc += dp[i];
        if (cc > count || (cc == count && cur < price))
            price = cur,
            count = cc,
    ...
    {
        if (pizzas[i].size() == 2)
            updateAns(pizzas[i][0], pizzas[i][1], i);
        FOR(j, i + 1, M)
            if (pizzas[i].size() && pizzas[j].size())
                updateAns(pizzas[i][0], pizzas[j][0], i | j);
    }
    writeln(ans);
}
