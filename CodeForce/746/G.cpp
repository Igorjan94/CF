...
void run()
{
    ...
    vector<pii> edges;
    auto minimize = [&](int layer, int count) {
        FOR(i, count, a[layer + 1])
            edges.pb({index[layer], index[layer + 1] + i});
    };
    auto maximize = [&](int layer, int count) {
        int cc = min(a[layer], a[layer + 1]) - 1;
        int currIndex = max(0, cc - count);
        //writeln(cc, currIndex, layer, count);
        fori(currIndex + 1)
            edges.pb({index[layer] + i, index[layer + 1] + i});
        minimize(layer, currIndex + 1);
        //writeln("returned", cc - currIndex);
        return cc - currIndex;
    };
