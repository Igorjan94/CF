...
void run()
{
    ...
    fori1(n + 1)
    {
        int d1 = a[i - 1].second - a[i - 1].first + 1;
        int d2 = a[i].second - a[i].first + 1;
        int c1 = a[i - 1].second / p - (a[i - 1].first - 1) / p;
        int c2 = a[i].second / p - (a[i].first - 1) / p;
        ll temp = (c1 * 1ll * d2 + c2 * 1ll * d1 - c1 * 1ll * c2);
        ans += 2000.0 * temp / d1 / d2;
    }
    cout.precision(13);
    cout << fixed << ans << "\n";
}
