...
{
    ints(n);
    vector<pii> a(n), zeros;
    vector<bool> used(n, false);
    vector<vi> lists;
    readln(a);
    fori(n)a[i].first--,a[i].second--;
    fori(n)
    {
        if (a[i].first >= 0)
            a[a[i].first].second = i;
        if (a[i].second >= 0)
    ...
    fori(n)
        if (!used[i])
        {
            ...
            lists.pb({j});
            while (a[j].second >= 0)
                lists[sz].pb(a[j].second),
                used[a[j].second] = true,
                j = a[j].second;
            j = i;
            while (a[j].first >= 0)
                lists[sz].insert(lists[sz].begin(), a[j].first),
                used[a[j].first] = true,
                j = a[j].first;
        }
    //writeln();
    //writeln(lists);
    fori(int(lists.size()) - 1)
        a[lists[i].back()].second = lists[i + 1][0],
        a[lists[i + 1][0]].first = lists[i].back();
