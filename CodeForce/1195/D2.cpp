...
mod solve(vector<string>& a)
{
    ...
    fori(n)
        forj(l * 2)
            ans += degs[j] * a[i][SZ(a[i]) - 1 - j / 2];
    return ans * n;
}

void run()
{
    fori(22) degs.pb(degs.back() * 10);

    ints(n);
    vector<string> a(n);
    readln(a);
    fori(n) for (char& c: a[i]) c -= 48;
        ...
        for (auto& q: m)
            if (p.first < q.first)
            {
                ...
                for (string s: q.second)
                {
                    ...
                    reverse(whole(t));
                    temp.pb(t);
                    int left = q.first - p.first;
                    forj(left)
                        pq += degs[left - j - 1] * s[j];
                }
                pq *= degs[p.first * 2];
                pq *= SZ(p.second);
                pq *= 2;
