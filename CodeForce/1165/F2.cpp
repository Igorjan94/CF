...
void run()
{
    ...
    readln(k, d);
    vector<set<int>> s(n);
    for (auto& [x, y]: d) s[--y].insert(-x);

    int ans = binSearch(0, 100000 * 4 + 1, [&](int time) {
        vector<int> need = k;
        vector<vector<int>> w(time + 1);
        fori(n)
            if (auto it = s[i].lower_bound(-time); need[i] && it != s[i].end())
                w[-*it].pb(i);
        int b = 0;
        fori1(time + 1)
        {
        ...
        return b - accumulate(whole(need), 0) * 2 >= 0;
    }, 1);
    writeln(ans);
}
