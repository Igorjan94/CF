...
//printAnswerAndExit
template<typename T>void printAnswerAndExit(T a){writeln(a);exit(0);}
//Igorjan

string to7(int x, int n)
{
    string s = "";
    fori(n)
        s += x % 7,
...
void run()
{
    ...
    ll value1 = 7;
    ll value2 = 7;
    while (value1 < n) deg1++, value1 *= 7;
    while (value2 < m) deg2++, value2 *= 7;
    if (deg1 + deg2 > 7)
    ...
    fori(n)
        ...
        {
            set<char> ss;
            for (char c : (to7(i, deg1) + to7(j, deg2)))
                ss.insert(c);
            if (ss.size() == deg1 + deg2)
                ans++;
        }
    writeln(ans);
}
