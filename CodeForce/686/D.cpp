...
int dfs(int u)
{
    ...
    {
        for (auto v : edges[u])
            if (children[v] * 2 > children[u])
            {
                centroid[u] = centroid[v];
                while ((children[u] - children[centroid[u]]) * 2 > children[u])
                    centroid[u] = parent[centroid[u]];
            }
        if (centroid[u] == 0)
            centroid[u] = u;
    }
...
void run()
{
    ...
    parent.resize(n, 0);
    fori(n - 1)
        readln(p),edges[--p].pb(i + 1),parent[i + 1] = p;
    dfs(0);
    fori(q)
        readln(p),
        writeln(centroid[--p] + 1);
}
