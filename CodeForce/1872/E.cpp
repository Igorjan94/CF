...
struct Node 
{
    ...
    Node() : v(0), l(0), pos(0) {}
    Node(int v, int pos) : v(v), l(0), pos(pos) {}
    Node(int v, int l, int pos) : v(v), l(l), pos(pos) {}

    Node operator+(const Node& b) const {
        return Node(v ^ b.v, l ^ b.l, pos);
    }
...
void run()
{
    ...
    string s;
    readln(s);
    segmentTree<int, Node, Update> tree(a);
    fori(n)
        if (s[i] == '0')
            tree.update(i, i, 1);
    ints(q);
