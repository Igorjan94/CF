...
void run()
{
	...
	readln(p, c);
    for (int& x: c) --x;
    vector<set<pii>> potentials(5000);
    fori(n)
        potentials[p[i]].insert({c[i], i});

    ints(d);
    fori(d)
    {
        ints(x); --x;
        potentials[p[x]].erase({c[x], x});
        if (false) // {{{
        {
            forj(3)
            {
                cout << "POT: " << j << " ";
                for (auto& [club, _] : potentials[j])
                    cout << club << " ";
                writeln();
        ...
        while (true)
        {
            for (auto& [club, _] : potentials[mex])
                s.insert(club);
            if (mex >= SZ(s) || SZ(potentials[mex]) == 0)
                break;
            mex++;
        }
        writeln(mex);
