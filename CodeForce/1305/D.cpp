...
void run()
{
    ...
    while (children.size() > 1)
    {
        ...
        for (int i = 0; i + 1 < SZ(children); i += 2)
            next_children.pb(ask(children[i], children[i + 1]));
        if (children.size() % 2)
            next_children.pb(children.back());
        children = next_children;
        sort(whole(children));
        children.erase(unique(whole(children)), children.end());
    }
    ans(children[0]);
