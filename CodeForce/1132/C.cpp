...
void run()
{
    ...
    vector<int> b(n, 0);
    int ans = 0;
    fori(q) FOR(j, a[i].first, a[i].second + 1) b[j]++;

    fori(q)
    {
        auto get = [&](int j) {
            return b[j] - (j >= a[i].first && j <= a[i].second);
        };
        vector<int> pref(1, 0);
        forj1(n) pref.pb(pref.back() + (get(j) == 1));
        int sum = 0;
        forj(n) sum += !!get(j);

        forj(q) if (i != j)
            ans = max(ans, sum - pref[a[j].second] + pref[a[j].first - 1]);
    }
    writeln(ans);
}
