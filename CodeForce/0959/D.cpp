...
void run()
{
    ...
    vector<int> a(n);
    readln(a);
    auto sieve = linearSieve(2141469);
    vector<bool> used(2141469, false);

    auto check = [&](int x)
    {
        auto&& fact = sieve.factorization(x);
        for (auto& y : fact)
            if (used[y])
                return make_pair(false, move(fact));
        return make_pair(true, move(fact));
    };

    fori(n)
    {
        int x = a[i];
        auto [ok, fact] = check(x);
        if (ok)
            for (auto& y : fact)
                used[y] = true;
        else
        {
            do x++; while (!check(x).first);
            auto fact = sieve.factorization(x);
            for (auto& y : fact)
                used[y] = true;
            a[i] = x;
