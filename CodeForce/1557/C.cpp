...
{
    ints(n, k);
    vector<modular<>> degs = {1};
    vector<modular<>> facs = {1, 1};
    vector<modular<>> ifacs = {1, 1};
    fori(k + 10)
        degs.pb(degs.back() * 2);
    FOR(i, 2, n + 10)
        facs.pb(facs.back() * i),
        ifacs.pb(ifacs.back() / i);
    ...
    fori(k)
    {
        if (n % 2 == 0) // It's case when all bits ==1
            ans += temp * (degs[k - i - 1] ^ n),
            temp *= even - 1;
        else
            temp *= even + 1;
    }
    writeln(ans + temp);
    //writeln(n, k, ans + temp, bf(n, k));
}
