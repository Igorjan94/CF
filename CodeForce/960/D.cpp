...
void run()
{
    ints(n);
    int levels = 63;
    vector<ll> degs(1, 1ll);
    vector<ll> shiftValues(levels);
    fori(levels - 1) degs.pb(degs.back() * 2ll);

    auto getLevel = [&](ll x) {
        fori(degs.size())
            if (degs[i] > x)
                return i - 1;
        return -1 + int(degs.size());
    };
    ...
    fori(n)
    {
        ...
        if (type == 1)
        {
            ...
            k %= degs[level]; if (k < 0) k += degs[level];
            shiftValues[level] = (shiftValues[level] + k) % degs[level];
        } else if (type == 2) {
            readln(x, k);
            int level = getLevel(x);
            k %= degs[level]; if (k < 0) k += degs[level];
            for (; level < levels; ++level)
                shiftValues[level] = (shiftValues[level] + k) % degs[level], k *= 2;
        } else {
            vector<ll> ans;
            readln(x);
            int level = getLevel(x);
            ans.pb(x);
            while (x) {
                x = (x - degs[level] + shiftValues[level]) % degs[level] + degs[level];
                x /= 2;
                level--;
                if (level == -1)
                    break;
                x = (x - shiftValues[level]) % degs[level] + degs[level];
                ans.pb(x);
            }
            writeln(ans);
        }
