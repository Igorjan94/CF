...
struct Node 
{
    ...
    Node(int v, int pos, int l) : v(v), pos(pos), l(l) {}
...
void run()
{
    ...
    vi a(n);
    readln(a);
    persistentSegmentTree<int, Node, Update> tree(vector(n + 1, 0));
    map<int, queue<int>> m;
    vector<int> roots = {tree.roots[0]};
    fori(n)
    {
        auto& q = m[a[i]];
        q.push(i + 1);
        tree.update(tree.roots.back(), i + 1, i + 1, 1);
        if (q.size() > k)
            tree.update(tree.roots.back(), q.front(), q.front(), -1),
            q.pop();
        roots.pb(tree.roots.back());
    }
    ll last = 0;
    ints(q);
    forn(Q, q)
