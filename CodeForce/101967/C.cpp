...
void run()
{
    ...
    vector<int> pref(1, 0);
    fori1(n + 1) pref.pb(pref.back() + a[i].first);

    vector<vector<int>> dp(n + 1, vector<int>(T + 1, -MOD));
    fori(n + 1)
        dp[i][0] = 0;
    forj1(T + 1)
        dp[0][j] = 0;
    fori1(n + 1)
        FOR(s, 1, T + 1)
            ...
            {
                if (s >= pref[i])
                    dp[i][s] = dp[i - 1][s - a[i].first] - a[i].second;
                else
                    dp[i][s] = max(dp[i - 1][s], dp[i - 1][s - a[i].first] - a[i].second);
            }
            else
                dp[i][s] = dp[i - 1][s];
    //writeln(pref);
