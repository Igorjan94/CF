...
struct action
{
    ...
    action(int type, int t, int l) : type(type), t(t), l(l) {}

    bool operator<(action const& b) const {
        return tuple(t, type, l) < tuple(b.t, b.type, b.l);
    }
};

void addAns(ll& x, ll y)
{
    x = (x + y) % MOD;
}
...
void run()
{
    ...
    for (auto& [l, r] : a)
        actions.pb(action(1, l, r - l)),
        actions.pb(action(-1, r + 1, 0));
    sort(whole(actions));
    multiset<int> free;
    ll ans = 0;
    for (auto& [type, t, l] : actions)
        if (type == -1)
            free.insert(-t);
        else
        {
