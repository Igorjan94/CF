...
struct modularR
{
    ...
    template<typename T1> inline modularR& operator*=(T1 const& t)       { value = (value * 1ll * t.value) % mod; return *this; }
...
void run()
{
    modularR phi(2, MOD - 1);
    ints(k);
    if (k == 1) return writeln(6);
    phi ^= k;
    phi -= 2;
    writeln((modular<int>(4) ^ phi.value) * 6);
}
