...
void run()
{
    ...
    string s;
    readln(s);
    vector<vector<int>> dp(3, vector<int>(n + 4, 0));
    fori(n)
    {
        dp[0][i + 3] = dp[0][i] + (s[i] == 'R');
        dp[1][i + 3] = dp[1][i] + (s[i] == 'G');
        dp[2][i + 3] = dp[2][i] + (s[i] == 'B');
    }
    int ans = n + 1;
    fori(n - k + 1)
    {
        auto get = [&](int index, int mx, int need) {
            int ik = index + k / 3 * 3 - 9;
            while (ik + 3 < mx) ik += 3;
            return dp[need][ik + 3] - dp[need][index];
        };
        ans = min(ans, k - get(i + 0, i + k, 0) - get(i + 1, i + k, 1) - get(i + 2, i + k, 2));
        ans = min(ans, k - get(i + 0, i + k, 1) - get(i + 1, i + k, 2) - get(i + 2, i + k, 0));
        ans = min(ans, k - get(i + 0, i + k, 2) - get(i + 1, i + k, 0) - get(i + 2, i + k, 1));
    }
    writeln(ans);
}
