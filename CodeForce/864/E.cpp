...
struct f
{
    int t, d, p, i;
    f(){}
    bool operator<(f& b) const { return d < b.d; }
};
...
void run()
{
    ...
    forn(k, n)
        fori(2002)
        {
            ...
            {
                int j = i - a[k].t;
                if (dp[k + 1][i] <= dp[k][j] + a[k].p)
                    dp[k + 1][i] = dp[k][j] + a[k].p,
                    prev[k + 1][i] = {a[k].i, j};
            }
        }
    int ans = 0;
    ...
    ROF(i, n, 1)
    {
        if (prev[i][index].first > 0)
            items.pb(prev[i][index].first);
        index = prev[i][index].second;
    }
    writeln(items.size());
    reverse(whole(items));
    writeln(items);
...
istream&operator>>(istream&is,f&a){return is>>a.t>>a.d>>a.p;}
ostream&operator<<(ostream&os,f&a){return os<<a.t<<" "<<a.d<<" "<<a.p;}
