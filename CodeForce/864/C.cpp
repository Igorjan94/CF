...
void run()
{
    ...
    fori(k)
    {
        ll toFinish = (k - i) * 1ll * a;
        if (fuel >= toFinish) //We have enough fuel to finish
            break;
        ll currentZ, nextZ;
        if (i % 2 == 0) // ->
        {
            currentZ = f;
            nextZ = currentZ + 2 * (a - f);
            if (currentZ > fuel) //Not enough fuel to zzz
        ...
        if (fuel < 0) //We didn't get to end of race
            printAnswerAndExit(-1);
    }
    writeln(ans);
}
