...
pointtt pair<T,T> dist(point<T>&a,point<T>&b,point<T>&c){return{abs(c.x*(b.y-a.y)-c.y*(b.x-a.x)+b.x*a.y-a.x*b.y),dist(a,b)};}
...
void run()
{
    ...
    fori(n)
    {
        auto p = dist(points[i], points[i + 2], points[i + 1]);
        mn = min(mn, fabs(p.first) / sqrt(p.second) / 2);
    }
    cout.precision(10);
    cout << fixed << mn << "\n";
}
