...
vector<int> g[MAXN];
bool used[MAXN];
int timer, tin[MAXN], fup[MAXN], maxSize[MAXN], rest[MAXN];
int answer = INF;
int n, m;
	...
	for (int to : g[v])
		if (to != p)  
            ...
            else 
            {
                ...
                if (fup[to] >= tin[v])
                    maxSize[v] = max(maxSize[v], t),
                    rest[v] -= t;
            }
    maxSize[v] = max(maxSize[v], rest[v]);
    return cc;
}
...
void run()
{
    ...
    fori(m)
        ...
        g[y].pb(x);
    dfs(0);
    writeln(min_element(maxSize, maxSize + n) - maxSize + 1);
}
