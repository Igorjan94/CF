...
void run()
{
    ...
    vi a(n);
    readln(a);
    vector<int> last(x + 2, -1);
    vector<int> first(x + 2, -1);
    fori(n) last[a[i]] = i + 1;
    ROF(i, n - 1, 0) first[a[i]] = i + 1;
    ll ans = 1;
    vector<pii> prefix;
    int ll = 0;
    for (int j = 1; j <= x && (first[j] == -1 || ll < first[j]); ++j)
    {
        if (first[j] != -1)
            ll = last[j];
        prefix.emplace_back(ll, j);
        ans++;
    }
    if (ans == x + 1)
        return writeln(x * 1ll * (x + 1) / 2);

    ll = n + 2;
    int i = SZ(prefix) - 1;
    for (int j = x; j >= 1 && (first[j] == -1 || ll > last[j]); --j)
    {
        if (first[j] != -1)
            ll = first[j];
        while (i >= 0 && prefix[i].first > ll) --i;
        if (i >= 0)
            ans += prefix[i].second;
        ans++;
    }
