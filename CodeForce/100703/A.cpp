...
void run()
{
    ...
    vector<string> a(n);
    readln(a);
    vector<vector<pii>> edges(n + 3);
    int temp;
    fori(n)
        FOR(j, i + 1, n)
            temp = dist(a[i], a[j]),
            edges[i + 1].pb({j + 1, temp}),
            edges[j + 1].pb({i + 1, temp});
    //fori1(n + 1)
    //{
        //wr(i);
        //writeln(edges[i]);
    ...
    fori1(27)
        {
            ...
            while (true)
            {
                ...
                used[u] = true;
                forj(edges[u].size())
                    if (edges[u][j].second <= i)
                        if (!used[edges[u][j].first])
                            q.insert(edges[u][j].first),
                            used[edges[u][j].first] = true;
            }
            bool ok = true;
            forj1(n + 1)
                ok &= used[j];
