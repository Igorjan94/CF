...
[[maybe_unused]] const int MOD = 998244353;
...
void run()
{
    ...
    string s;
    readln(s);
    vector<vector<modular<>>> degs(N + 1);
    vector<vector<modular<>>> p(1 << N, vector<modular<>>(N + 1));
    vector<modular<>> ans(1 << N);
    fori(N + 1)
    {
        degs[i].pb(1);
        forj(n + 1)
            degs[i].pb(degs[i].back() * i);
    }
    //int sum = 0;
    //for (char c: s) sum += c == '?';
    fori(n)
