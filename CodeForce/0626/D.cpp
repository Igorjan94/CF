...
void run()
{
    ...
    fori(n)
        FOR(j, i + 1, n)
            m[a[j] - a[i]]++;
    //writeln_range(whole(m));
    vector<pii> b;
    vector<ll> c(m.size() + 1, 0);
    ll ans = 0;
    for (auto& x : m)
        b.pb(x);
    ROF(i, b.size() - 1, 0)
        c[i] = c[i + 1] + b[i].second;
    for (auto& x : b)
        for (auto& y : b)
        {
            ...
            int m;
            while (r - l > 1)
                b[m = (r + l) / 2].first <= sum ? l = m : r = m;
            b[l].first <= sum ? m = r : m = l;
            ans += x.second * 1ll * y.second * c[m];
        }
    int sz = c[0];
    cout.precision(10);
    cout << fixed << (ans * 1.0 / sz / sz / sz) << "\n";
}
