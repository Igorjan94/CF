...
bool dfs(int i, int j, int pi, int pj, int ppi, int ppj)
{
    if (i < 0 || j < 0 || i >= used.size() || j >= used[0].size() || a[i][j] != color || (i == ppi && j == ppj))
        return false;
    if (used[i][j])
        return cur >= 4;
    used[i][j] = true;
    cur++;
    return dfs(i - 1, j, i, j, pi, pj) || dfs(i + 1, j, i, j, pi, pj) || dfs(i, j - 1, i, j, pi, pj) || dfs(i, j + 1, i, j, pi, pj);
}
...
void run()
{
    ...
    fori(n)
        forj(m)
            if (!used[i][j])
                if ((cur = 1) && (color = a[i][j]) && dfs(i, j, -1, -1, -1, -1))
                {
                    writeln("Yes");
                    return;
                }
