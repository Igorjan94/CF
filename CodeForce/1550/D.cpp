...
void run()
{
    ints(n, l, r);
    modular<int> ans;
    vector<modular<int>> fac = {1, 1};
    vector<modular<int>> ifac = {1, 1};
    fori(n)
        fac.pb(fac.back() * (i + 2)),
        ifac.pb(ifac.back() / (i + 2));
    auto cnk = [&](int n, int k) {
        if (k < 0) return modular<int>(0);
        if (k == 0) return modular<int>(1);
        return fac[n] * ifac[n - k] * ifac[k];
    };
    for (int x = n / 2; x <= n; ++x)
    {
        int posValue = r - x;
        int cnt = n - (l - posValue);
        if (cnt < n - x) continue;
        int common = x + cnt - n;
        int onlyPositive = x - common;
        int needsMorePositive = n / 2 - onlyPositive;
        ans += cnk(common, common - needsMorePositive);
    }
    int xxx = min(r - n, -l);
    ans += cnk(n, n / 2) * xxx;
    if (n % 2 == 1)
