...
void run()
{
    ...
    fori(n)
        readln(x),
        c.emplace_back(x, i + 1);
    sort(whole(c));
    bool found;
    vector<int> first;
    vector<int> second;
    auto getCount = [&](int x, int i) {
        return x / c[i].first + (x % c[i].first != 0);
    };
    
    auto find = [&](int x1, int x2) {
        found = false;
        first.clear();
        second.clear();
        vector<int> can(n + 1, -1);
        for (int i = n - 1; i >= 0; --i)
            can[i] = (c[i].first * 1ll * (n - i) >= x2) ? i : can[i + 1];

        fori(n)
        {
            int cc = getCount(x1, i);
            int nn = i + cc;
            if (nn < n && can[nn] != -1)
            {
                found = true;
                FOR(j, i, nn)
                    first.pb(c[j].second);
                int cc2 = getCount(x2, can[nn]);
                FOR(j, can[nn], can[nn] + cc2)
                    second.pb(c[j].second);
                break;
            }
        }
    };
    ...
    {
        writeln("Yes");
        writeln(first.size(), second.size());
        writeln(first);
        writeln(second);
    }
    else
