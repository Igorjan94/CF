...
bool isInside(circle& a, circle& b)
{
    ll dx = abs(a.x - b.x);
    ll dy = abs(a.y - b.y);
    return b.r > a.r && dx * dx + dy * dy <= b.r;
}
...
void run()
{
    ...
    cout.precision(15);
    cout << fixed << (accumulate(a.begin(), a.end(), 0ll, [&](ll acc, circle c) {
        return acc + c.r * ((max(1, accumulate(a.begin(), a.end(), 0, [&](int inside, circle d) {
            return inside + isInside(c, d);
        })) % 2) * 2 - 1);
    }) * M_PI) << "\n";
}
