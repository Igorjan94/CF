...
ll f(ll k)
{
    int count = 0;
    ll ans = 0;
    for (auto it = a.begin(); it != a.end() && *it < k; ++it, count = (count + 1) % m)
        if (count == 0)
            ans += 2 * (k - *it);
    count = 0;
    for (auto it = a.rbegin(); it != a.rend() && *it > k; ++it, count = (count + 1) % m)
        if (count == 0)
            ans += 2 * (*it - k);
    return ans;
}
...
void run()
{
    ...
    while (b - a > 2);
    writeln(min(min(f(a), f(a - 1)), min(f(a + 1), f(a+2))));
}
