...
{
    return x1 * x1 - 8. * x1 + x2 * x2 + 1. * (x1 * x1 + (x2 - 4) * (x2 - 4) - 9) * (x1 * x1 + (x2 - 4) * (x2 - 4) - 9);
}
...
//pair<double, double> fastestGradientDown()
{
    ...
    auto fun = [&x1, &x2, &fx1, &fx2, &f](double lambda){return f(x1 - lambda * fx1(x1, x2), x2 - lambda * fx2(x1, x2));};
