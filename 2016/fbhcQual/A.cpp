...
#define  FILENAME    "boomerang_constellations"
...
void run(int test, vector<pii> points)
{
    TYPE ans = 0;
    ...
    for (auto& a : points)
    {
        ...
        for (auto& b : points)
            if (a != b)
                m[(a.first - b.first) * 1ll * (a.first - b.first) + (a.second - b.second) * 1ll * (a.second - b.second)]++;
        for (auto x : m)
