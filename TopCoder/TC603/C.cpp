...
#define CLASSNAME GraphWalkWithProbabilities
#define METHODNAME findprob
#define PARAMETRES vector <string> g, vector <int> w, vector <int> f, int s
#define TYPE double
...
double dfs(vi& w, vi& f, int u)
{
    used[u] = true;
    prob[u] = (double)w[u] / (w[u] + f[u]);
    fori(e[u].size())
        if (!used[e[u][i]])
            prob[u] = max(prob[u], (double) w[u] / 100 + ((double) (100 - w[u] - f[u]) * dfs(w, f, e[u][i]) / 100));
    return prob[u];
}
