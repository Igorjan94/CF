...
void run(int test, ll n)
{
    map<int, int> dp2;
    dp2[1] = 1;
    int temp;
    while (dp2.find(n) == dp2.end())
    {
        map<int, int> dp = dp2;
        for (auto x : dp)
        {
            int i = x.first;
            if (dp2.find(i + 1) == dp2.end())
                dp2[i + 1] = x.second + 1;
            else
                dp2[i + 1] = min(dp2[i + 1], x.second + 1);
            temp = reverse(i);
            if (temp > n)
                continue;
            if (dp2.find(temp) == dp2.end())
                dp2[temp] = x.second + 1;
            else
                dp2[temp] = min(dp2[temp], x.second + 1);
        }
    }
    answer[test] = dp2[n];
