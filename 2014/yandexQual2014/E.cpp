...
void run()
{
    ...
    pr2[1] = pr2[2] = a[1];
    for(int i = 2; i < n; i += 2)
        pr1[i] = pr1[i + 1] = pr1[i - 2] + a[i];
    for(int i = 3; i < n; i += 2)
        pr2[i] = pr2[i + 1] = pr2[i - 2] + a[i];

    for(int i = 1; i < n; i++)
        pr3[i] = pr2[i - 1] - pr1[i - 1];
    for(int j = 1; j < n; j++)
        pr4[j] = pr1[j] - pr2[j];
    ll mx3 = *max_element(whole(pr3));
    ll mx4 = *max_element(whole(pr4));
    ll mn3 = *min_element(whole(pr3));
    ll mn4 = *min_element(whole(pr4));
    writeln(max(abs(mx3 + mx4), abs(mn3 + mn4)));
}
