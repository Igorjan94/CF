...
for i in $(ls); do
    ...
    exiv2 -v -M"set Exif.Image.Make $(exiv2 -p e $f | grep Exif.Image.Make | head -1 | awk {'{for (i=4; i<NF; i++) printf $i " "; print $NF}'})" $o 1>/dev/null 2>&1
    exiv2 -v -M"set Exif.Image.Model $(exiv2 -p e $f | grep Exif.Image.Model | head -1 | awk {'{for (i=4; i<NF; i++) printf $i " "; print $NF}'})" $o 1>/dev/null 2>&1
    exiv2 -v -M"set Exif.Image.Software $(exiv2 -p e $f | grep Exif.Image.Software | head -1 | awk {'{for (i=4; i<NF; i++) printf $i " "; print $NF}'})" $o 1>/dev/null 2>&1
    exiv2 -v -M"set Exif.Image.DateTime $(exiv2 -p e $f | grep Exif.Image.DateTime | head -1 | awk {'{for (i=4; i<NF; i++) printf $i " "; print $NF}'})" $o 1>/dev/null 2>&1
    exiv2 -v -M"set Exif.Image.Artist $(exiv2 -p e $f | grep Exif.Image.Artist | head -1 | awk {'{for (i=4; i<NF; i++) printf $i " "; print $NF}'})" $o 1>/dev/null 2>&1
