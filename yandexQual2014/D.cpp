...
string run()
{
    ...
    fori(n)
        readln(a[i].x, a[i].y);
    pii maxx = a[0], maxy = a[0];
    forn1(i, n)
        if (f(a[i], maxx) || f(a[i], maxy))
            return "Invisible paper detected";
        else
        {
            if (maxx.x > a[i].x || maxx.x == a[i].x && maxx.y >= a[i].y)
                maxx = a[i];
            if (maxy.y > a[i].y || maxy.y == a[i].y && maxy.x >= a[i].x)
                maxy = a[i];
        }
    return "Well done";
}
