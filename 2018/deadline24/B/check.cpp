...
int main(int argc, char** argv)
{
    ...
    ints(t);
    vector<vector<vector<int>>> field(t);
    vector<vector<pair<pii, pii>>> pipes(t);
    forn(q, t)
    {
        ints(x, y, n);
        field[q].resize(x, vi(y, 0));
        fori1(n + 1)
        {
            ints(bx, by, ex, ey);
            field[q][--bx][--by] = i; --ex; --ey;
            pipes[q].pb({{bx, by}, {ex, ey}});
        }
    }
    ...
    fori(t)
    {
        ...
        ints(n);
        if (pipes[i].size() != n)
            printf("Fail in test %d\n", i + 1);
        forj1(n + 1)
        {
            string s;
            readln(s);
            auto [b, e] = pipes[i][j - 1];
            auto [x, y] = b;
            for (char c : s)
            {
                if (c == 'W')
                ...
                if (c == 'S')
                    y++;
                if (field[i][x][y])
                    printf("Field[%d][%d] should be %d'th pipe, but pipe %d already exists\n", x, y, j, field[i][x][y]);
                field[i][x][y] = j;
            }
            if (x != e.first || y != e.second)
                printf("Pipe %d ends in wrong place: %d %d instead of %d %d\n", j, x, y, e.first, e.second);
        }
    }
    return 0;
}
