...
#define CLASSNAME SplitIntoPairs
#define METHODNAME makepairs
#define PARAMETRES vector<int> a, int x
#define TYPE int
...
class CLASSNAME
{
    public :
        TYPE METHODNAME(PARAMETRES)
        {
            ...
            sort(a.begin(), a.end());
            int i = 0;
            while (i < n - 1 && 1ll * a[i] * a[i + 1] >= 0)
                count++,
                i += 2;
            if (i >= n)
                return count;
            int j = n - 1;
            while (j > i && 1ll * a[j] * a[j - 1] >= 0)
                count++,
                j -= 2;
            if (1ll * a[i] * a[j] >= x)
                return count + 1;
