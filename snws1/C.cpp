...
pair<ll, ll> dfs(int u, int p = -1)
{
    ll mx = 0, mx2 = 0, temp = 0, temp2 = 0;
    fori(edges[u].size())
        if (edges[u][i].first != p)
        {
            auto x = dfs(edges[u][i].first, u);
            temp = x.second + edges[u][i].second;
            temp2 = max(temp2, x.first);
            if (mx <= temp)
                mx2 = mx,
                mx = temp;
            else
...
{ 
    ...
    pair<ll, ll> d;
    d = dfs(0);
    mx = max(mx, max(d.first, d.second));
    writeln(mx);
}
