...
void run()
{
    ints(m, n, p, k, s);
    vi ans(m, k);
    auto getDriver = [&](int d) {
        return d + m;
    };
    vector<vector<pii>> edges(n + m);
    fori(s)
    {
        ints(d, x, r);
        edges[getDriver(d)].pb({x, r});
    }
    ints(t);
    fori(t)
    {
    ...
    fori(n)
    {
        int driver = getDriver(i);
        vector<int> d(n + m, MOD);
        d[driver] = 0;
        ...
        fori(m)
            if (d[i] != MOD)
                ans[i] = max(1, ans[i] - (p - d[i]));
    }
    writeln(*min_element(whole(ans)));
}
