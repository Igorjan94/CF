...
struct dsu
{
    vector<ll> a, prices, edges;

    dsu(int n, vector<ll>& prices)
        : prices(prices)
    {
        a.resize(n);
        edges.resize(n, 0ll);
    ...
    void uni(int i, int j, ll w)
    {
        i = get(i);
        j = get(j);
        if (i != j && w < max(prices[i], prices[j]))
            a[i] = j,
            edges[j] += edges[i] + w,
            prices[j] = min({prices[i], prices[j]});
    }
};
...
void run()
{
    ...
    readln(a);
    ints(k);
    vector<tuple<ll, int, int>> edges(n);
    dsu d(n, a);
    fori(k)
    {
        ints(x, y, w); --x; --y;
