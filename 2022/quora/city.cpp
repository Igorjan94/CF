...
void run()
{
    ...
    readln(r);
    vector<ll> ans(n);
    function<void(chtTrick<ll>, int, int, int)> dfs = [&](chtTrick<ll> cht, int u, int p, ll d) {
        ans[u] = cht.get(d);
        cht.addLine(r[u].first, r[u].second + ans[u] - r[u].first * d);
        for (auto [v, w]: g[u])
            if (v != p)
                dfs(cht, v, u, d + w);
    };
    chtTrick<ll> cht(true);
    cht.addLine(r[0].first, r[0].second);
    dfs(cht, 0, -1, 0);
    ans.erase(ans.begin());
    writeln(ans);
}
